...
var pullData = function(settings, data, functioncall){
    var newData = data;
    newData.forEach(function(d,i){
        //var newValue = d.value + Math.floor((Math.random()*10) - 5);
        var newValue = Math.random() * 100 | 0;
        //newData[i].value = newValue <= 0 ? 10 : newValue;
        newData[i].value = newValue;
    });
    newData = formatData(newData);

    functioncall(settings,newData);
};

var render_update = function(settings, data, callback) {
